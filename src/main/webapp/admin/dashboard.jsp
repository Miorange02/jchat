...
<script>
    ...
    $('.sidebar-item').click(function() {
        ...
        if (page === 'users') {
            ...
            $('#usersContent').removeClass('hidden'); // 显示用户管理内容
        } else if (page === 'chatrooms') {
            $('#pageTitle').text('聊天室管理');
            $('#pageDescription').text('管理系统中的所有聊天室');
            $('#chatroomsContent').removeClass('hidden'); // 显示聊天室管理内容
        } else if (page === 'messages') {
            $('#pageTitle').text('消息记录');
            $('#pageDescription').text('查看和管理系统中的所有消息记录');
            $('#messagesContent').removeClass('hidden'); // 显示消息记录内容
        }
    });

    // 页面加载时默认显示用户管理内容（新增代码）
    ...
    function showEditUserModal(id) {
        ...
        $.get('/admin/getUser', {id: id}, function(response) {
            ...
            if (typeof response === 'object' && response.success && response.data) {
                ...
                console.log('响应数据不符合预期，提示获取失败');
                alert(response.message || '获取用户数据失败，请重试！');
            }
        }).fail(function(jqXHR, textStatus, errorThrown) {
            console.error('AJAX请求失败，状态:', textStatus, '，错误信息:', errorThrown);
            alert('获取用户数据失败，请重试！');
        });
    }
    // 关闭编辑用户模态框
    ...
    $('#addChatroomBtn').click(function() {
        $('#addChatroomModal').removeClass('hidden');
    });
